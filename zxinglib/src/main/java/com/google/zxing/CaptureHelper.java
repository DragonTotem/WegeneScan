...
import com.google.zxing.beep.BeepManager;
import com.google.zxing.camera.AmbientLightManager;
...
public class CaptureHelper implements CaptureLifecycle, CaptureTouchEvent, CaptureManager, SurfaceHolder.Callback {
    ...
    /**
     * 用于跳转相册然后返回处理，需要暂时暂停解析数据
     */
    private boolean pauseToHandle;

    /**
     * use {@link #CaptureHelper(Fragment, SurfaceView, BaseViewfinderView, View)}
     *
     * @param fragment
     * @param surfaceView
     ...
     */
    @Deprecated
    public CaptureHelper(Fragment fragment, SurfaceView surfaceView, BaseViewfinderView viewfinderView) {
        this(fragment, surfaceView, viewfinderView, null);
    }

    public CaptureHelper(Fragment fragment, SurfaceView surfaceView, BaseViewfinderView viewfinderView, View ivTorch) {
        this(fragment.getActivity(), surfaceView, viewfinderView, ivTorch);
    }

    /**
     * use {@link #CaptureHelper(Activity, SurfaceView, BaseViewfinderView, View)}
     *
     * @param activity
     * @param surfaceView
     * @param viewfinderView
     */
    @Deprecated
    public CaptureHelper(Activity activity, SurfaceView surfaceView, BaseViewfinderView viewfinderView) {
     ...
     * @param ivTorch
     */
    public CaptureHelper(Activity activity, SurfaceView surfaceView, BaseViewfinderView viewfinderView, View ivTorch) {
    ...
    private void initCamera(SurfaceHolder surfaceHolder) {
        ...
        try {
            ...
            if (captureHandler == null) {
                captureHandler = new CaptureHandler(activity, viewfinderView, onCaptureListener,
                        decodeFormats, decodeHints, characterSet, cameraManager, pauseToHandle);
            ...
            LogUtil.w("Unexpected error initializing camera", e.getMessage());
        }
    }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        if (holder == null) {
            LogUtil.w("*** WARNING *** surfaceCreated() gave us a null surface!");
        }
        if (!hasSurface) {
            hasSurface = true;
            initCamera(holder);
    ...
    public CaptureHelper setIvTorchView(View ivTorch) {
        this.ivTorch = ivTorch;
        if (ivTorch != null && hasCameraFlash) {
            ivTorch.setOnClickListener(v -> {
                if (cameraManager != null) {
    ...
    public void resetScanViewState() {
        if (captureHandler != null) {
            captureHandler.setScanViewState();
        }
    }
    ...
    public void stopPreviewAndDecode() {
        ...
        if (captureHandler != null) {
            captureHandler.stopPreviewAndDecode();
        }
    }

    public void setPauseToHandle(boolean pauseToHandle) {
        this.pauseToHandle = pauseToHandle;
    }
