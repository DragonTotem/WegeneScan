...
public abstract class BaseViewfinderView extends View {
     ...
     * 扫码框高宽占比
     */
    private float frameHeightWidthRatio = 1f;
    /**
     * 扫码框距离顶部有少距离
     */
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        isShowResultPoint = array.getBoolean(R.styleable.ScanViewfinderView_showResultPoint, false);

        frameWidth = array.getDimensionPixelSize(R.styleable.ScanViewfinderView_frameWidth, 0);
        frameHeight = array.getDimensionPixelSize(R.styleable.ScanViewfinderView_frameHeight, 0);

        laserStyle = LaserStyle.getFromInt(array.getInt(R.styleable.ScanViewfinderView_laserStyle, LaserStyle.LINE.mValue));
        gridColumn = array.getInt(R.styleable.ScanViewfinderView_gridColumn, 20);
        gridHeight = (int) array.getDimension(R.styleable.ScanViewfinderView_gridHeight, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40, getResources().getDisplayMetrics()));

        cornerRectWidth = (int) array.getDimension(R.styleable.ScanViewfinderView_cornerRectWidth, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 4, getResources().getDisplayMetrics()));
        cornerRectHeight = (int) array.getDimension(R.styleable.ScanViewfinderView_cornerRectHeight, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 16, getResources().getDisplayMetrics()));
        scannerLineMoveDistance = (int) array.getDimension(R.styleable.ScanViewfinderView_scannerLineMoveDistance, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, getResources().getDisplayMetrics()));
        scannerLineHeight = (int) array.getDimension(R.styleable.ScanViewfinderView_scannerLineHeight, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, getResources().getDisplayMetrics()));
        frameLineWidth = (int) array.getDimension(R.styleable.ScanViewfinderView_frameLineWidth, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, getResources().getDisplayMetrics()));
        scannerAnimationDelay = array.getInteger(R.styleable.ScanViewfinderView_scannerAnimationDelay, 15);
        frameRatio = array.getFloat(R.styleable.ScanViewfinderView_frameRatio, 0.712f);
        frameHeightWidthRatio = array.getFloat(R.styleable.ScanViewfinderView_frameHeightWidthRatio, 1f);
        framePaddingTop = (int) array.getDimension(R.styleable.ScanViewfinderView_framePaddingTop, 0);
        array.recycle();
    ...
    protected void initFrame() {
        ...
        if (frameHeight <= 0 || frameHeight > screenHeight) {
            if (frameHeightWidthRatio == 1) {
                frameHeight = size;
            } else {
                frameHeight = (int) (size * frameHeightWidthRatio);
            }
        }
     ...
     * @param frame
     */
    protected void drawTextInfo(Canvas canvas, Rect frame) {
     ...
     * @param frame
     */
    protected void drawCorner(Canvas canvas, Rect frame) {
     ...
     * @param frame
     */
    protected void drawLaserScanner(Canvas canvas, Rect frame) {
        if (laserStyle != null) {
     ...
     * @param frame
     */
    protected void drawLineScanner(Canvas canvas, Rect frame) {
        //线性渐变
     ...
     * @param frame
     */
    protected void drawGridScanner(Canvas canvas, Rect frame) {
        int stroke = 2;
     ...
     * @param frame
     */
    protected void drawFrame(Canvas canvas, Rect frame) {
     ...
     * @param height
     */
    protected void drawExterior(Canvas canvas, Rect frame, int width, int height) {
     ...
     * @param frame
     */
    protected void drawResultPoint(Canvas canvas, Rect frame) {
     ...
     * 暂停绘制
     */
    public void setNoSuspendDrawLaser(boolean noSuspendDrawLaser) {
        this.noSuspendDrawLaser = noSuspendDrawLaser;
    }
